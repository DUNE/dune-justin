...
def getMetadata(fileName, fileSize, fileAdler32, 
  ...
  for keyName in list(metadata['metadata']).copy():
...
if resultsResponseDict['status'] != 200:
  logLine('record_results returns: ' + str(resultsResponseDict['response']))
...
with open(os.environ.get('RUCIO_HOME') + '/etc/rucio.cfg', 'w') as f:
  f.write('''[client]
