...
SUBCOMMANDS
       ...
       create-request  [--description  DESC] [--mql QUERY|--monte-carlo COUNT]
              ...
              Carlo instances to  run.  If  a  description  is  not  given,  a
              description  based on a timestamp is created.  Requests are cre-
              ated in the state  "draft"  and  the  command  returns  the  new
              request's  ID number.  Once the request is in the running state,
              justIN will use the MQL expression to find  the  list  of  input
              files from MetaCat. If --refind-interval-hours is given, the MQL
              query will be resubmitted at that interval to add any new match-
              ing files from the start of the day given by --refind-start-date
              (default: today  in  UTC)  for  the  number  of  days  given  by
              --refind-duration-days (default: 1).
       ...
       create-stage --request-id ID --stage-id ID --jobscript  FILENAME|--job-
              ...
              [--max-distance DIST] [--max-files-per-job  N]  [--scope  SCOPE]
              [--output-pattern  DATASET:PATTERN] [--output-pattern-next-stage
              DATASET:PATTERN]  [--output-rse  NAME]  [--lifetime-days   DAYS]
              [--env NAME=VALUE]
              ...
              Pattern  Matching  section for details. The --scope SCOPE option
              is also required. Any output files found by the pattern matching
              will  be uploaded and registered by the generic job in the Rucio
              dataset given by SCOPE:DATASET, with a DID composed of the scope
              given  and  the  found filename. Furthermore, if the found files
              have a corresponding JSON metadata file with the same  name  but
              with  ".json"  appended,  that will be recorded for that file in
              MetaCat.
       ...
       quick-request [--description DESC]  [--mql  QUERY|--monte-carlo  COUNT]
              [--refind-start-date   YYYYMMDD]  [--refind-duration-days  DAYS]
              [--refind-interval-hours  HOURS]   --jobscript   FILENAME|--job-
              script-id   JSID   [--wall-seconds  N]  [--max-files-per-job  N]
              [--rss-mb N] [--processors  N]  [--max-distance  DIST]  [--scope
              SCOPE]  [--output-pattern  DATASET:PATTERN]  [--output-rse NAME]
