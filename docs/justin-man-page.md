...
SUBCOMMANDS
       ...
       create-stage  --request-id  ID  --stage-id  ID   --jobscript   FILENAME
              ...
              boostrap shell script associated with it, given  by  the  --job-
              script  option which will be executed on worker nodes to process
              its files.
       ...
       create-jobscript  [--description  DESC]  [--scope  SCOPE]  --name  NAME
              ...
              under  your  username.  The  jobscript identifier is returned on
              success, in the form SCOPE:NAME or USER:NAME.   Jobscript  names
              must  be  unique  for  each  scope  or  username. If a jobscript
              already exists for the given scope or username it  is  overwrit-
              ten.

       show-jobscript [--scope SCOPE] --name NAME
       show-jobscript --request-id ID --stage-id ID
              Show  a jobscript, referenced either by name and optional scope,
              or by request and stage. If a name is given,  the  jobscript  is
              taken  from the Jobscripts Library. If a name is given without a
              scope, the command looks for the jobscript under your  username.
