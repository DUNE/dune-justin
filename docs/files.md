...
Within the context of its workflow/stage, an input file exists in one of the
following states:
...
For output files which are not inputs for subsequent stages, the states are
available:
...
- **finished** - the file was either directly uploaded to scratch or 
  was uploaded to Rucio-managed storage and successfully registered in 
  MetaCat and Rucio

For output files which are **not** inputs for subsequent stages, the output
files go through the recorded, registering (if for Rucio managed storage), 
and finished states.
